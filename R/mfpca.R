mfpca <- function(fdobj,center=TRUE){
  call = match.call()
  if (class(fdobj)=="list"){
    ...
    if (length(fdobj)>2) {
      ...
      pcafd<-list(call=call,eigval=valeurs_propres,harmonics=fonctionspropres,scores=scores,varprop=varprop,meanfd=mean_fd)
